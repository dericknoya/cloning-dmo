...
def analyze_segments(segments_df, activations_df, user_map):
    ...
    for _, row in tqdm(segments_df.iterrows(), total=segments_df.shape[0], desc=f"{get_timestamp()} Analisando Segmentos"):
        ...
        if status:
            results.append({
                "DELETAR": "NAO", "ID_OR_API_NAME": row['id'], "OBJECT_TYPE": "SEGMENT",
                "LAST_REFRESH_DATE": row['lastpublishstatusdatetime_dt'], "DELETION_IDENTIFIER": row['id'], 
                "DISPLAY_NAME": row['name'], "STATUS": status, "Reason": reason, 
                "CREATED_BY_NAME": user_map.get(row['createdbyid'], row['createdbyid']),
                "OBJECT_MAPPING_ID": "", "FIELD_MAPPING_ID": ""
            })
...
def analyze_activations(activations_df, segment_info_map):
    ...
    for _, row in tqdm(grouped.iterrows(), total=grouped.shape[0], desc=f"{get_timestamp()} Analisando Ativações Relacionadas"):
        ...
        results.append({
            ...
            "CREATED_BY_NAME": "N/A", "OBJECT_MAPPING_ID": "", "FIELD_MAPPING_ID": ""
        })
        
    print(f"{get_timestamp()} Análise de Ativações Relacionadas concluída.")
    return results

def analyze_dmos(dmos_df, dmo_details_df, activations_df, ci_expression_df, user_map, all_segment_criteria_text, mappings_df):
    if dmos_df.empty: return []

    mapping_dict = {}
    if not mappings_df.empty and all(col in mappings_df.columns for col in ['sourceentitydevelopername', 'developername', 'fieldmappings.developername']):
        mappings_df['fieldmappings.developername'] = mappings_df['fieldmappings.developername'].fillna('')
        mapping_info = mappings_df.groupby('sourceentitydevelopername').agg({
            'developername': lambda x: ';'.join(x.unique()),
            'fieldmappings.developername': lambda x: ';'.join(x)
        }).reset_index()
        mapping_dict = mapping_info.set_index('sourceentitydevelopername').to_dict('index')
    ...
    if not activations_df.empty:
        for col in ['activationtargetsubjectconfig.developername', 'attributesconfig.attributes.entityname', 'contactpointsconfig.contactpoints.contactpointentityname']:
            if col in activations_df.columns:
                dmos_used_in_activations.update(activations_df[col].dropna())
    ...
    for _, row in tqdm(dmos_df.iterrows(), total=dmos_df.shape[0], desc=f"{get_timestamp()} Analisando DMOs"):
        ...
        mapping_data = mapping_dict.get(dmo_name, {})
        object_mapping_id = mapping_data.get('developername', '')
        field_mapping_id = mapping_data.get('fieldmappings.developername', '')
        
        created_by_name = user_map.get(row.get('createdbyid'), "N/A")
        results.append({
            "DELETAR": "NAO", "ID_OR_API_NAME": row['name'], "LAST_REFRESH_DATE": "", 
            "OBJECT_TYPE": "DMO", "DELETION_IDENTIFIER": row['name'], 
            "DISPLAY_NAME": row['label'], "STATUS": "ORFAO", "Reason": reason, 
            "CREATED_BY_NAME": created_by_name, "OBJECT_MAPPING_ID": object_mapping_id, 
...
def analyze_data_streams(streams_df, stream_details_df, mappings_df, user_map):
    ...
    for _, row in tqdm(streams_with_details.iterrows(), total=streams_with_details.shape[0], desc=f"{get_timestamp()} Analisando Data Streams"):
        ...
        mapping_data = mapping_dict.get(dlo_name, {})
        object_mapping_id = mapping_data.get('developername', '')
        field_mapping_id = mapping_data.get('fieldmappings.developername', '')
        
        has_mapping = dlo_name in mapping_dict
        ...
        if status:
            ...
            results.append({
                "DELETAR": "NAO", "ID_OR_API_NAME": dlo_name, 
                "LAST_REFRESH_DATE": row['lastrefreshdate_dt'], "OBJECT_TYPE": "DATA_STREAM", 
                "DELETION_IDENTIFIER": dlo_name, "DISPLAY_NAME": display_name, 
                "STATUS": status, "Reason": reason, "CREATED_BY_NAME": created_by_name,
                "OBJECT_MAPPING_ID": object_mapping_id, "FIELD_MAPPING_ID": field_mapping_id
            })
...
def analyze_calculated_insights(ci_df, user_map):
    ...
    for _, row in tqdm(ci_df.iterrows(), total=ci_df.shape[0], desc=f"{get_timestamp()} Analisando Calculated Insights"):
        ...
        if is_successful and is_older_than_90_days:
            created_by_name = user_map.get(row.get('createdbyid'), "N/A")
            results.append({
                "DELETAR": "NAO", "ID_OR_API_NAME": row['apiname'], 
                "LAST_REFRESH_DATE": row['lastrundatetime_dt'], "OBJECT_TYPE": "CALCULATED_INSIGHTS", 
                "DELETION_IDENTIFIER": row['apiname'], "DISPLAY_NAME": row['displayname'], 
                "STATUS": "INATIVO", "Reason": "Último processamento bem-sucedido > 90 dias.", 
                "CREATED_BY_NAME": created_by_name, "OBJECT_MAPPING_ID": "", "FIELD_MAPPING_ID": ""
            })
...
def main():
    ...
    dmo_results = analyze_dmos(dfs.get("dmos", pd.DataFrame()), dfs.get("dmo_details", pd.DataFrame()), activations_df, dfs.get("ci_expression", pd.DataFrame()), user_map, all_segment_criteria_text, mappings_df)
    stream_results = analyze_data_streams(dfs.get("streams", pd.DataFrame()), dfs.get("stream_details", pd.DataFrame()), mappings_df, user_map)
    ...
    # Define a ordem final das colunas, incluindo as novas
    final_columns = [
        "DELETAR", "ID_OR_API_NAME", "LAST_REFRESH_DATE", "OBJECT_TYPE", 
        "DELETION_IDENTIFIER", "DISPLAY_NAME", "OBJECT_MAPPING_ID", 
        "FIELD_MAPPING_ID", "STATUS", "Reason", "CREATED_BY_NAME"
    ]
