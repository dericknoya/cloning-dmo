...
Versão: 9.0 - Logs de Sucesso/Falha no padrão de deleteObjects.py
...
- Para cada campo a ser excluído:
  - Remove o mapeamento associado, tratando erros 404 e 412 de forma inteligente.
  - Tenta deletar o campo usando o ID da coluna 'DELETION_IDENTIFIER'.
  - Trata erros 500 com uma verificação posterior.
- Relatório Final: Ao final da execução, exibe um resumo no console e gera dois
  arquivos CSV, seguindo o padrão do script de deleção de objetos:
    1. 'success_delete_fields.csv': Com os campos deletados com sucesso.
    2. 'fail_delete_fields.csv': Com os campos que falharam e o motivo.
    ...
    try:
        async with session.delete(delete_url) as response:
            if response.status in [204, 404]:
                ...
                reason = "Falha de pré-condição (412): O campo é referenciado em outra funcionalidade (ex: Calculated Insight). A dependência deve ser removida manually na plataforma."
                return False, reason
...
# AJUSTE: Funções de log para seguir o padrão de deleteObjects.py
def write_success_csv(successful_deletions):
    """Gera um arquivo CSV com a lista de campos deletados com sucesso."""
    filename = "success_delete_fields.csv"
...
def write_failure_csv(failed_deletions):
    """Gera um arquivo CSV com os detalhes das falhas na deleção."""
    filename = "fail_delete_fields.csv"
...
async def mass_delete_fields(file_path, dry_run):
    ...
    async with aiohttp.ClientSession(headers=headers, proxy=proxy_url) as session:
        ...
        # AJUSTE: Gera os arquivos CSV com os novos nomes e funções
        if not dry_run:
            if successful_deletions:
                write_success_csv(successful_deletions)
            if failed_deletions:
                write_failure_csv(failed_deletions)
