...
DMO_PREFIXES_TO_EXCLUDE = ('ssot', 'unified', 'individual','Individual', 'einstein', 'Einstein', 'segment_membership', 'AA_', 'aa_', 'aal_', 'AAL_', 'ibb_', 'iub_')
...
def main():
    ...
    files_to_load = {
        "dmos": "DataModelObjects.csv", "dmo_details": "DataModelObjectsDetails.csv",
        "users": "Users.csv", "mkt_fields": "MktDataModelField.csv",
        "cis_expression": "CIExpression.csv", "segment_filters": "SegmentFilters.csv",
    ...
    segment_fields = set()
    segments_df = dfs.get('segment_filters', pd.DataFrame())
    if not segments_df.empty:
        include_cols = ['include.objectapiname', 'include.fieldapiname']
        if all(col in segments_df.columns for col in include_cols):
            for _, row in segments_df.dropna(subset=include_cols).iterrows():
                segment_fields.add((str(row[include_cols[0]]).lower(), str(row[include_cols[1]]).lower()))
        exclude_cols = ['exclude.objectapiname', 'exclude.fieldapiname']
        if all(col in segments_df.columns for col in exclude_cols):
            for _, row in segments_df.dropna(subset=exclude_cols).iterrows():
                segment_fields.add((str(row[exclude_cols[0]]).lower(), str(row[exclude_cols[1]]).lower()))
    ...
    print(f"✅ Total de {len(all_used_fields)} pares (DMO, Campo) únicos encontrados em uso direto.")
    ...
    if all_fields_df.empty:
        ...
        return
    print(f"\n   - LOG: Total de {len(all_fields_df)} campos de DMO encontrados para análise (antes dos filtros).")
        
    # --- INÍCIO DA NOVA LÓGICA DE ENRIQUECIMENTO DE MAPEAMENTOS ---
    print("\n   - LOG: Enriquecendo campos com informações de Mapeamento...")
    mappings_df = dfs.get("mappings", pd.DataFrame())
    if not mappings_df.empty and 'targetentitydevelopername' in mappings_df.columns:
        # Seleciona e renomeia as colunas de mapeamento para o merge
        mapping_info_df = mappings_df[[
            'targetentitydevelopername', 
            'fieldmappings.targetfielddevelopername',
            'developername', 
            'fieldmappings.developername'
        ]].rename(columns={
            'developername': 'OBJECT_MAPPING_ID',
            'fieldmappings.developername': 'FIELD_MAPPING_ID'
        })
        
        # Cria chaves para o merge (case-insensitive)
        all_fields_df['dmo_name_lower'] = all_fields_df['dmo_name'].str.lower()
        all_fields_df['field_name_lower'] = all_fields_df['field_name'].str.lower()
        mapping_info_df['targetentitydevelopername_lower'] = mapping_info_df['targetentitydevelopername'].str.lower()
        mapping_info_df['fieldmappings.targetfielddevelopername_lower'] = mapping_info_df['fieldmappings.targetfielddevelopername'].str.lower()
        
        # Faz o left merge para adicionar as colunas de mapeamento
        all_fields_df = pd.merge(
            all_fields_df, 
            mapping_info_df,
            left_on=['dmo_name_lower', 'field_name_lower'],
            right_on=['targetentitydevelopername_lower', 'fieldmappings.targetfielddevelopername_lower'],
            how='left'
        )
        
        # Remove colunas temporárias
        all_fields_df.drop(columns=['dmo_name_lower', 'field_name_lower', 'targetentitydevelopername', 'fieldmappings.targetfielddevelopername', 'targetentitydevelopername_lower', 'fieldmappings.targetfielddevelopername_lower'], inplace=True, errors='ignore')
        print("   - LOG: Informações de mapeamento adicionadas.")
    else:
        all_fields_df['OBJECT_MAPPING_ID'] = None
        all_fields_df['FIELD_MAPPING_ID'] = None
        print("   - LOG: Arquivo de mapeamentos não encontrado ou inválido. Colunas de mapeamento ficarão vazias.")
    # --- FIM DA NOVA LÓGICA DE ENRIQUECIMENTO DE MAPEAMENTOS ---
    ...
    main_pk_fields = set(kq_field.replace('kq_', '', 1) for kq_field in kq_fields)
    ...
    all_fields_df = all_fields_df.drop(columns=['field_name_lower'])
    ...
    for _, row in all_fields_df.iterrows():
        ...
        records.append({
            ...
            'OBJECT_MAPPING_ID': row.get('OBJECT_MAPPING_ID'),
            'FIELD_MAPPING_ID': row.get('FIELD_MAPPING_ID'),
            'Status': status
        })
    ...
    # A coluna DELETION_IDENTIFIER não é relevante para campos em uso
